...
<Div id="contenidos">
		<header>
			<h2>CONSULTA FACTURAS DIRECTAS </h2>
		</header>

	
<?php
echo '<table border= "1">';
	echo '<td><h3>No FACTURA<h3></td><td><h3>PAGO/DEBE<h3></td><td><h3>ANULADO<h3></td><td><h3>Fecha</h3></td><td><h3>Placa/Identidad</h3></td><td><h3>VISTA IMPRESION</h3></td><td><h3>GENERAR PDF</h3></td>'; 
		while($ordenes = mysql_fetch_array($consulta_ordenes))
			{ 
				...
				echo  '<td><h3>';
				echo '<a href="otro_index.php?id_factura='.$ordenes['3'].'"  target = "_blank" >Generar Pdf</a>';
				echo '</h3></td>'; 
				echo  '<td><h3>';
				echo '<a href="otro_index_item.php?id_factura='.$ordenes['3'].'"  target = "_blank" >Generar Pdf</a>';
				echo '</h3></td>'; 
