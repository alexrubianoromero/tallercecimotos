...
function mostrar_items($factupan)
				{		
						...
						$sql_mostrar_items ="select * from $tabla100 where no_factura = '".$factupan."'";

						$consulta_mostrar_item  = mysql_query($sql_mostrar_items,$conexion);

						$numero_filas = mysql_num_rows($consulta_mostrar_item);
						echo '<br>';
						echo '<table border = 1 width = "'.$_SESSION['ancho_tabla'].'">';
						echo '<tr>';
						echo '<td>	ITEM </td>';
						echo '<td>	CODIGO </td>';
						...
						if ($numero_filas > 0 )
						{
							...
							echo '<tr>';
							  ...
							  echo '<td>'.$d['total_item_con_iva'].'</td>';
							  ...
							  echo '<tr><td></td><td></td><td></td><td>TOTAL.</td><td>';
							   echo $total_remision;
							   echo '<input type = "hidden"  id="total_remision"  value="'.$total_remision.'" > ';
							   echo '</td></tr></table>';
