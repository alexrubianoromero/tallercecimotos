...
echo '<tr>';
echo '<td>USUARIO</td>';
echo '<td>FECHA</td><td>RECIBIDO DE </td><td>CONCEPTO</td><td>VALOR</td><td>EFECTIVO</td><td>T_DEBITO</td><td>T_CREDITO</td></tr>';
...
while($recibos = mysql_fetch_assoc($consulta_recibos))
		{
			$datos_usuario = consulta_assoc($tabla16,'id_usuario',$recibos['id_usuario_creacion'],$conexion);
			...
			echo '</tr>';
			$total_entradas = $total_entradas + $recibos['lasumade'];
			$total_efectivo = $total_efectivo + $recibos['efectivo'];
			$total_t_debito = $total_t_debito + $recibos['t_debito'];
			$total_t_credito = $total_t_credito + $recibos['t_credito'];
...
echo '<tr>';
			...
			echo '<td align="right">'.number_format($total_entradas, 0, ',', '.').'</td>';
			echo '<td align="right">'.number_format($total_efectivo, 0, ',', '.').'</td>';
			echo '<td align="right">'.number_format($total_t_debito, 0, ',', '.').'</td>';
			echo '<td align="right">'.number_format($total_t_credito, 0, ',', '.').'</td>';
			echo '</tr>';
echo '</table>';
...
echo '<tr>';
echo '<td>USUARIO</td>';
echo '<td>FECHA</td><td>PAGADO A  </td><td>CONCEPTO</td><td>VALOR</td><td>EFECTIVO</td><td>T_DEBITO</td><td>T_CREDITO</td></tr>';
$traer_recibos_del_dia = "select * from $tabla23 where id_empresa = '".$_SESSION['id_empresa']."' and fecha_recibo = '".$abiertos['fecha']."' and anulado = '0'  ";
